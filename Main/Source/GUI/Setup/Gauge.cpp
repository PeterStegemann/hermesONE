...
GUI_Setup_Gauge::GUI_Setup_Gauge( void)
			   : left( 0)
			   , top( 0)
			   , width( 0)
			   , height( 0)
...
void GUI_Setup_Gauge::calculateInnerDimensions( void)
{
...
void GUI_Setup_Gauge::SetDimensions( uint16_t Left, uint16_t Top, uint16_t Width, uint16_t Height)
{	
	left = Left;
	top = Top;
	width = Width;
...
void GUI_Setup_Gauge::SetOptions( Options UseOptions)
{
	options = UseOptions;
...
void GUI_Setup_Gauge::SetColors( LCD_65K_RGB::Color ForegroundColor,
								 LCD_65K_RGB::Color BackgroundColor, LCD_65K_RGB::Color DetailColor)
{
...
void GUI_Setup_Gauge::SetBackgroundColor( LCD_65K_RGB::Color BackgroundColor)
{
	backgroundColor = BackgroundColor;
	marker.BackgroundColor = BackgroundColor;
...
void GUI_Setup_Gauge::SetDetailColor( LCD_65K_RGB::Color DetailColor)
{
	detailColor = DetailColor;
	marker.ForegroundColor = DetailColor;
...
void GUI_Setup_Gauge::Display( void)
{
	Display( minimum, maximum, value);
}

void GUI_Setup_Gauge::Display( int16_t Value)
{
	Display( minimum, maximum, Value);
}

void GUI_Setup_Gauge::Display( int16_t Minimum, int16_t Maximum, int16_t Current)
{
	if( remembered == true)
	{
	...
	if( options & O_CenterLine)
	{
		if( options & O_Vertical)
		{
			GLOBAL.SetupDisplay.DrawHorizontalLine( left + 1, centerPosition, width - 2,
												    foregroundColor);
		}
		else
		{
			GLOBAL.SetupDisplay.DrawVerticalLine( centerPosition, top + 1, height - 2,
												  foregroundColor);
		}
	}
}

void GUI_Setup_Gauge::Clear( void)
{
	// Display marker or normal?
	if( options & O_Marker)
	{
