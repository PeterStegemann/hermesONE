...
class Setup_Service
{
	...
	public:
		...
		// Set proxy source.
		void SetProxyReference( uint8_t ModelId, uint8_t ProxyId,
							    const Setup_Source_Tupel* ProxyReference);
		void GetProxyReference( uint8_t ModelId, uint8_t ProxyId,
							    Setup_Source_Tupel* ProxyReference);
		// Same as above for selected model id.
		void SetProxyReference( uint8_t ProxyId, const Setup_Source_Tupel* ProxyReference);
		void GetProxyReference( uint8_t ProxyId, Setup_Source_Tupel* ProxyReference);
