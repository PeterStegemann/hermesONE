...
void Screen_Setup_Status::display( void)
{
	...
	statusGauge[ SETUP_STATUS_SOURCE_RIGHT_BOTTOM].SetDimensions( frameLeft + BottomGaugeSize + 3,
																  BottomGaugeTop, BottomGaugeSize,
																  SCREEN_SETUP_BASE_GAUGE_THICKNESS);
	...
	GLOBAL.SetupDisplay.Print( TextLeft, frameTop + ( Line++ * Font->CellHeight), FONT::FI_Large,
							   LCD_65K_RGB::C_White, LCD_65K_RGB::C_Black, LCD::PO_Proportional,
							   ModelName);
	...
	GLOBAL.SetupDisplay.Print( TextLeft, frameTop + ( Line++ * Font->CellHeight), FONT::FI_Large,
							   RFModeColor, LCD_65K_RGB::C_Black, LCD::PO_Proportional,
							   RFModeText);
	...
	for( uint8_t TimerId = 0; TimerId < SETUP_MODEL_STATUS_TIMERS; TimerId++)
	{
		...
		if( SetupSourceId == SETUP_SOURCE_NONE)
		{
			...
			{
				GLOBAL.SetupDisplay.Print_P( NameLeft, TextTop, FONT::FI_Large,
		...
		else
		{
			...
			GLOBAL.SetupDisplay.Print( NameLeft, TextTop,
									   FONT::FI_Large, LCD_65K_RGB::C_White, LCD_65K_RGB::C_Black,
